...
##
##  Layout: 8.5x11 in portrait with 0.75 in margins
##  (= 7.0 in usable width). Every figure and table is sized
##  to fit inside that printable area.
## ============================================================
...
## ---- Clean up labels so tables fit and read cleanly --------------------
##  The cumulative-relapse table has long auto-generated strata names
##  (e.g. "MAP3K8_grp=MAP3K8-high") that overflow the printable width.
cum_rel$group <- ifelse(grepl("high", cum_rel$group), "MAP3K8-high",
                 ifelse(grepl("low",  cum_rel$group), "MAP3K8-low",
                        as.character(cum_rel$group)))
cum_rel <- cum_rel[, c("group","time_months","S_t","CI_lower","CI_upper",
                       "relapse_pct")]
colnames(cum_rel) <- c("MAP3K8 group","Time (mo)","Survival",
                       "CI lower","CI upper","Relapse %")

##  The multivariable Cox table has long term names like
##  "RCB_fullRCB-0/I"; rename for readability.
mv_cox$term <- gsub("^MAP3K8_grpMAP3K8-",      "MAP3K8: ",      mv_cox$term)
mv_cox$term <- gsub("^RCB_fullRCB-",            "RCB: ",         mv_cox$term)
mv_cox$term <- gsub("^stage",                   "Stage ",        mv_cox$term)
mv_cox$term <- gsub("^age$",                    "Age (years)",   mv_cox$term)
colnames(mv_cox) <- c("Term","HR","95% LCL","95% UCL","P")

colnames(sub_hr) <- c("Subgroup","n","Events","HR","95% LCL","95% UCL","P")

## ---- Page-safe flextable helper ----------------------------------------
##  Set explicit column widths summing to <= 7 in, then add layout
##  autofit as a safety net. Returns a flextable object.
make_tbl <- function(df, widths) {
  ft <- flextable(df) |>
  ...
  for (j in seq_along(widths))
    ft <- width(ft, j = j, width = widths[j])
  if (ncol(df) > 1)
    ft <- align(ft, j = 2:ncol(df), align = "center")
  ft
}

## ---- Image-size helper -------------------------------------------------
##  Cap longest side to a safe maximum given 0.75 in margins (= 7.0 in
##  usable width, 9.5 in usable height).
img_size <- function(w, h, max_w = 6.5, max_h = 8.0) {
  r <- min(max_w / w, max_h / h, 1)
  list(width = w * r, height = h * r)
}
...
doc <- doc |>
  ...
  body_add_img(src = file.path(A8, "KM_MAP3K8_overall.png"),
               width = szA$width, height = szA$height) |>
  ...
  body_add_par("Cumulative distant relapse rates (Kaplan-Meier estimates).",
               style = "heading 3") |>
  ##  6 columns, widths sum to 7 inches
  body_add_flextable(make_tbl(cum_rel,
                              widths = c(1.4, 0.9, 1.0, 1.0, 1.0, 1.0))) |>
...
doc <- doc |>
  ...
  ##  5 columns: Term wider, others narrower
  body_add_flextable(make_tbl(mv_cox,
                              widths = c(2.4, 0.9, 1.0, 1.0, 0.9))) |>
...
doc <- doc |>
  ...
  body_add_img(src = file.path(A8, "KM_MAP3K8_RCB0I.png"),
               width = szC1$width, height = szC1$height) |>
...
doc <- doc |>
  ...
  body_add_img(src = file.path(A8, "KM_MAP3K8_RCBII_III.png"),
               width = szC2$width, height = szC2$height) |>
...
## ---- Analysis D (forest plot) ------------------------------------------
##  Native forest plot is 9 in wide x 4 in tall. Constrain width to 6.5 in;
##  height scales proportionally to ~2.9 in.
szD <- img_size(9, 4)
doc <- doc |>
  ...
  body_add_par(
    ...
    style = "Normal") |>
  body_add_par("Result.", style = "heading 3") |>
  ##  7 columns, widths sum to 7.0 in: Subgroup gets the largest slice
  body_add_flextable(make_tbl(sub_hr,
                              widths = c(2.4, 0.55, 0.65, 0.7, 0.85, 0.85, 0.9))) |>
  body_add_img(src = file.path(A8, "forest_subgroup_HRs.png"),
               width = szD$width, height = szD$height) |>
...
szE1 <- img_size(6.5, 6.5)
##  Native boxplot composite is 11 in wide x 12 in tall. Cap to 6.5 in
##  width so the page does not chop the right edge.
szE2 <- img_size(11, 12)
doc <- doc |>
  ...
  body_add_img(src = file.path(A7, "boxplots_MAP3K8_by_class.png"),
               width = szE2$width, height = szE2$height) |>
...
doc <- doc |>
  ...
  body_add_par(
    ...
    style = "Normal") |>
  body_add_par(
    "4. MAP3K8 expression does not predict initial chemoresponse (AUC 0.52; logistic OR 1.01, P = 0.96; Cohen's kappa vs Hatzis signature = 0.08). MAP3K8 therefore carries chemoresistance information that is orthogonal to standard predictive signatures.",
    style = "Normal") |>
  body_add_par(
...
## ---- Set page size + 0.75 in margins so nothing overflows --------------
ps <- prop_section(
...
doc <- body_set_default_section(doc, ps)
